...
- **Poka-yoke pass:** if the change adds a validator, detector, error message, or a "remember to X" note — ask why the bad state is representable at all. A layer-1 redesign (type/data/flow that makes it impossible) beats the proposed layer-2 check; name it as the simpler alternative.
