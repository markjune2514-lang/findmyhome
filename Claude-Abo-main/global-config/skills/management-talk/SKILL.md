...
Standalone document that lives on disk — handoff, design overview, internal wiki page, public README/blog. Closest to the JIRA-comment shape (bolded section labels, full structure) but as a complete document with its own header and footer.

**Before drafting, ask the audience mode and language** (one short combined question, then stop):

> *"Audience: Public / Internal-detailed / Internal-brief? Language: English / Thai / other?"*

Default if user doesn't answer: **internal-detailed** + **match the language the user has been speaking in this conversation**.

| Mode | Use for | Keep | Strip |
|---|---|---|---|
| **Public** | Release note, public README, blog, customer/community-facing | High-level capability, impact, what it does | File paths, line numbers, internal taxonomy codes (RT_xxx), function/struct names, hardcoded values (keys, blacklists, secrets), known-limitation lists that reveal attack surface, internal owner names |
| **Internal detailed** *(default)* | Handoff, post-mortem, design doc, onboarding for new teammate | Everything: file:line citations, mechanism, internal code tables, limitations as-is | Only true secrets (credentials) |
| **Internal brief** | Meeting handout, talking-point expansion, exec summary for internal lead | Bolded sections + tight bullets, "what it covers / what it doesn't / where the risk is" | Per-row code tables, deep mechanism, citations, debug history |

If user doesn't answer, default to **internal-detailed**.
...
**Last updated:** YYYY-MM-DD · **Owner:** <name/team>

**Status / TL;DR:** one line.

**Impact:** ...

**<other bolded sections as the JIRA-comment shape calls for>**

---
Source: <files/tickets> · Related: <links>
```

- H1 title at top, last-updated + owner line directly under.
- Bolded section labels (same building blocks as JIRA comment — Status, Impact, What broke / What it does, Why now, Owner, Next steps, Workaround, Risk — pick what fits).
- Horizontal rule + one-line footer (source files, related docs). Skip footer if there's nothing to cite.
- TOC only if document has >5 sections.

**Output path:** ask the user where to save. Default to `~/.claude/projects/<project-slug>/<TITLE>.md` when working inside a project; never write `.md` files into a code repo unless the user explicitly says so.
...
3. **Ask where it goes:**
   ...
   - Markdown file: write to disk via Write tool. Confirm path with user first (default `~/.claude/projects/<project-slug>/<TITLE>.md`).
